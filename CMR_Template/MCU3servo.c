...
#define SERVO_PER_UNIT 15 //15.6; 500 = 0.1 ms
...
#define NEUTRAL_SERVO_DUTY   5500
...
void __ISR(_TIMER_2_VECTOR, ipl2) Timer2Handler(void) { //empty ISR
    mT2ClearIntFlag(); //clear interrupt flag, if you forget to do this, the 
                        //microcontroller will interrupt continuously
}
...
void __ISR( _TIMER_3_VECTOR, ipl3) T3Interrupt( void){
    ...
    if (servo1_ON){
        ...
        WritePeriod3(NEUTRAL_SERVO_DUTY + Servo1_PWM * SERVO_PER_UNIT);
        servo1_ON = 1;
        mPORTASetBits(BIT_3);
    }
    mT3ClearIntFlag(); 
...
void __ISR( _TIMER_4_VECTOR, ipl3) T4Interrupt( void){
    ...
    if (servo2_ON){
        ...
        WritePeriod4(NEUTRAL_SERVO_DUTY + Servo2_PWM * SERVO_PER_UNIT);
        servo2_ON = 1;
        mPORTBSetBits(BIT_4);
    }
    mT4ClearIntFlag();  
...
void main(void) {
    ...
    //RPA3R = 0x05; //output compare 3, pin 10, Servo1
    //RPB4R = 0x05; //output compare 1, pin 11, Servo2

//    RPB11R = 0x05; //output compare 2, pin 22 TI_CUR
//    RPB10R = 0x05; //output compare 4, pin 21 T2_DIR
